...
export function FuelGauge({ percentage, canChange, onChange, showPer, ...props }) {
    if (percentage < 0) percentage = 0;
    else if (percentage > 100) percentage = 100;

    const svg = useRef();

    return (<svg ref={svg} onMouseDown={canChange && (({ clientX, clientY }) => onChange(calcPct(clientX - svg.current?.getBoundingClientRect().x, clientY - svg.current?.getBoundingClientRect().y, 40, 50)))} onMouseMove={canChange && (({ clientX, clientY, buttons }) => buttons === 1 && onChange(calcPct(clientX - svg.current?.getBoundingClientRect().x, clientY - svg.current?.getBoundingClientRect().y, 40, 50)))} viewBox="15 15 70 55" {...props}>
        ...
        <text className="select-none" x="75" y="49" textAnchor="middle" fontSize="11" fill="currentColor" fontWeight="bold">F</text>
        <text className="select-none" x="25" y="49" textAnchor="middle" fontSize="11" fill="currentColor" fontWeight="bold">E</text>

        {showPer && <text textAnchor="middle" x="50" y="50" fontSize="9" fill="currentColor">{percentage} %</text>}
...
function calcPct(x, y, cx, cy) {
    const x1 = x - cx;
    const y1 = y - cy;
    const angle = Math.atan(y1 / x1);
    let deg = radToDeg(angle);
    if (deg < 0) deg += 180;
    const newPercentage = Math.min(Math.max((deg - 50) / 80 * 100, 0), 100);
